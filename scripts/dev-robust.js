...
// Function to clean up only lock files (non-aggressive)
...
// Function to check for port conflicts without killing processes
function checkPortConflicts() {
  return new Promise((resolve) => {
    console.log('🔍 Checking for port conflicts...');
    // Just check, don't kill anything
    resolve();
  });
}
...
async function startDevServer() {
  try {
    console.log('🔍 Smart port detection (avoiding conflicts)...');
    
    // Clean up only lock files
    cleanupLockFiles();
    
    // Check for port conflicts (without killing processes)
